...
// import { loginUser } from '../redux/actions'; // Assuming this is where your login action is
...
//     } catch (err) {
//       const backendMessage = err.response?.data?.message || 'Registration failed'; // Backend error message
...
//     setError(''); // Reset error when Snackbar closes
//   };
...
const Register = () => {
  ...
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { user } = useSelector((state) => state.auth); // Get user from Redux after login
