...
const AddService = () => {
  ...
  const fetchWorkers = async () => {
    ...
    const ratingsData = ratingsResponses.reduce((acc, curr, index) => {
      acc[response.data[index]._id] = curr.data.rating; // map worker ID to their rating
      return acc;
    }, {});
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const errorMessage = err.response && err.response.data ? err.response.data.error : 'Error removing company';
  ...
  const handleSubmit = async () => {
    try {
      ...
      const errorMessage = err.response && err.response.data ? err.response.data.error : 'Error adding service';
      setSnackbarMessage(errorMessage);
      setSnackbarOpen(true);
    }
  };
  ...
  return (
    <Container sx={{ paddingY: 5, bgcolor: '#fafafa' }}>
      ...
      {user?.role === 'worker' && (
        <Button variant="contained" color="primary" onClick={handleOpenModal} sx={{ bgcolor: '#6a1b9a', color: '#fff' }}>
          Add Your Company
        </Button>
      )}
      {workers.length === 0 ? ( 
    <Typography variant="h6" align="center" sx={{ marginTop: 3, color: '#6a1b9a' }}>
      No Providers Found
    </Typography>
  ) :(
        ...
        {workers.map((worker) => (
          <Grid item xs={12} sm={6} md={4} key={worker._id}>
            ...
            <CardContent>
              ...
              <Typography variant="body2" color="textSecondary" sx={{ marginY: 0.5 }}>
            ...
            <Typography variant="body1" sx={{ display: 'flex', alignItems: 'center', gap: 1, marginY: 0.5 }}>
                Rating: {ratings[worker._id] !== undefined ? (
                  <ReactStars
                    count={5}
                    value={ratings[worker._id]} // Display fetched rating
                    size={24}
              ...
              <Typography variant="body1" sx={{ marginY: 1, fontSize: '1.2rem', color: '#333' }}>
                Price: ${worker.price}
              </Typography>
              {user?.role === 'user' && (
                <Button
      ...
      <Modal open={confirmDeleteModal} onClose={handleDeleteCancel}>
        <Box sx={{ width: 400, margin: 'auto', padding: 3, bgcolor: 'background.paper', marginTop: '10%', borderRadius: 2 }}>
          <Typography variant="h6" component="h2" sx={{ marginBottom: 2, color: '#6a1b9a' }}>
      ...
      <Modal open={openModal} onClose={handleCloseModal}>
        <Box sx={{ width: 400, margin: 'auto', padding: 3, bgcolor: 'background.paper', marginTop: '10%', borderRadius: 2 }}>
          ...
          <IconButton onClick={handleCloseModal} sx={{display:'flex', position:'relative',left:'47%',marginBottom: 2, justifyContent:'flex-end' }}>
              <CloseIcon />
            </IconButton>
            </Box>
          <TextField
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center', marginTop: 2 }}>
            <Button variant="contained" color="primary" onClick={handleSubmit} sx={{ bgcolor: '#6a1b9a', color: '#fff' }}>Submit</Button>
